...
/// Find col staring idx given any cell idx [0-81)
///
/// Expects parameters:
/// - `cell_id: u8`  - index of given cell
...
/// - `rowstart_cell_id: u8`  - representing the idx of the row start (leftmost position)
pub fn get_board_row(board: &[u8], rowstart_cell_id: u8) -> Vec<u8> {
    let mut row: Vec<u8> = Vec::new();
    let rci: usize = rowstart_cell_id as usize; // Must be cast to usize for indexing
    for b in 0..3 {
        for c in 0..3 {
...
/// Expects the parameters:
/// - `board: &[u8]`  - representing board in 1D array
/// - `colstart_cell_id: u8`  - representing the idx of the col start (topmost position)
pub fn get_board_col(board: &[u8], colstart_cell_id: u8) -> Vec<u8> {
    let mut col: Vec<u8> = Vec::new();
    let cci: usize = colstart_cell_id as usize; // must be cast to usize for indexing
    for b in 0..3 {
        for c in 0..3 {
...
/// - `board: &[u8]`  - representing board in 1D array
/// - `rowstart_cell_id: u8`  - Representing the Idx of the row start (leftmost position)
pub fn print_one_row(board: &[u8], rowstart_cell_id: u8) {
    println!("Y X  0, 1, 2,  3, 4, 5,  6, 7, 8");
    print!("{}  ", get_cell_row(rowstart_cell_id));
    let row = get_board_row(board, rowstart_cell_id);
    for i in 0..9 {
...
pub fn print_one_col(board: &[u8], colstart_cell_id: u8) {
    println!("X Y  0, 1, 2,  3, 4, 5,  6, 7, 8");
    print!("{}  ", get_cell_col(colstart_cell_id));
...
/// - `board: &[u8]`  - representing board in 1D array
/// - `rowstart_cell_id: u8`  - Representing the Idx of the row start (leftmost position)
pub fn print_row(board: &[u8], rowstart_cell_id: u8) {
    let row = get_board_row(board, rowstart_cell_id);
    for i in 0..9 {
        let cell = if *row.get(i).unwrap() == 0 {
            ' '
